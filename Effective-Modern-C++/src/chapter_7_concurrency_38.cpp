 ...
 * 
 * 重要ポイント
 * - 単純なイベント通信の場合、条件変数を用いた設計では余分な mutex が必要になり、検知タスク、反応タスクに成約が生まれる。さらに反応タスクでは
 *   イベントが実際に発生したかを確認する必要もある。
 * - フラグを用いた設計では上記問題点を回避できるが、ポーリング動作でありブロック動作ではない。
 * - 条件変数とフラグの併用も可能だが、やや大げさな構造の通信となる。
 * - std::promise と future を用いた設計では上記問題点を回避できるが、共有ステート用のヒープメモリを消費する。さらに通信が 1 度切りという制限が
 *   ある。
 * 
