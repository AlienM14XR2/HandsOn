 ...
 * - 既知の型、ムーブ対応可能の型を使用する場合は、上記想定は不要である。
 * 
 * ※ 本項はコーディングよりも Reading が主になりそうだだな。
 * ※ うん、最後までコーディングすることはなかったな、うん、この章は重要だと思うが、終始こんな感じだし
 *    何らなムーブセマンティクスに関しても良い印象は持っていない（扱いづらいしプログラマは別のことに注意が行くことがある、設計や実装詳細）。
 *    対応したとて、そのメリットが明確に提示されていないことも良くないのかもしれない。究極的な言い方をするなら、そんなに処理スピードと効率
 *    にこだわるなら C を使えよって私は思う、自分ならそうする。
 * 
