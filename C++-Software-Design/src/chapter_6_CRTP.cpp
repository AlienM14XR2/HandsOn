 ...
 * 
 * C++20 では、CRTP パターンよりも、より簡潔で既存ソースにも干渉しない、コンセプトを検討すべきです。もちろん、C++20 の
 * コンセプトが使える環境ではなく、かつ CRTP パターンが適していれば、CRTP パターンを採用すべきです。
 * 
 * ガイドライン 26 の要約
 * - コンパイル時に型ファミリの関係を定義する目的で、CRTP パターンを適用する。
 * - CRTP 基底クラスから派生クラスへのアクセスには制限があることを理解する。
 * - CRTP パターンの制約、特に共通する基底クラスが存在しない点を肝に銘じておく。
 * - 可能であれば、CRTP パターンよりも C++20 のコンセプトを優先する（オイッ：）
 * 
 * ガイドライン 27 ：静的 mixin クラスには CTRP パターン （このサンプルはおそらく実装しない：）
 * ガイドライン 27 の要約
 * - デザインパターンとしての CRTP と、実装パターンとしての CRTP を使用する場合の差異を認識する。
 * - 抽象化する CRTP 基底クラスはデザインパターンとしても機能することを理解する。
 * - 抽象化しない CRTP 基底クラスは実装パターンとして機能することを理解する。
 * 
 * e.g. compile
