...
## ガイドライン１１の要約
- デザインパターンは独立性を高める目的を持ち、名前を持ち、かつ検証済の解であることを理解する。
- デザインパターンはなんらかの形で抽象化することを認識する。
...
## ガイドライン１２の要約
- デザインパターンは設計上の問題を解決する手段であり、最終目的と考えてはいけません。
- デザインパターンはオブジェクト指向プログラミングに限定されるものではない点に注意する。
- デザインパターンは動的多態性に限定されるものではないことを忘れてはいけない。
...
- 必要に応じ、目的に即したデザインパターンを使用する。

# デザインパターン名を用い目的を伝える

## ガイドライン１４の要約
- 解の目的を伝えるため、デザインパターン名を使用する（ソース上でも）。
- 読解性を向上させるため、デザインパターン名を使用する。

では、次回は具体的にその「デザインパターン」を学んでいこう、今度は書籍（C++）でね。