 ...
 * 
 * Iterator パターンは、内部実装を公開せずにアグリゲートの要素の走査を可能にする。
 * また、Iterator パターンは、走査処理をアグリゲートではなくイテレータに任せる。
 * これにより、アグリゲートインタフェースも実装もシンプルに保ち、本来の責務を遂行できる
 * ようにする。
 * 
 * 単一責務の原則
 * クラスの責務は、変更される可能性のある部分に及ぶ。
 * 複数の責務を持つことは、変更される可能性のある部分が複数あることを意味する。
 * 
 * この原則は、クラスに１つの責務だけを与えるように指導している。
 * 
 * e.g. compile.
